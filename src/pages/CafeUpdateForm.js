...
function CafeUpdateForm() {
     ...
     useEffect(()=>{
         ...
         // window가 resize 될때마다 동작할 리스너 함수 등록하기
         //window.addEventListener("resize", handleResize)
         console.log("CafeUpdateForm이 활성화됨")
         // useEffect()에 전달한 함수 안에서 리턴해주는 
         // 함수는 해당 컴포넌트가 비활성화 되는 시점에 호출된다. 
         // 즉, 해당 컴포넌트에서 무언가 마무리 작업을 할 게 있으면 리턴해주는 함수 안에서 작업 하면된다.
         return ()=>{
            console.log("CafeUpdateForm이 언제 호출되지")
            // 이벤트 리스너 제거하기 
            window.addEventListener("resize", handleResize)
         }
     }, [])
