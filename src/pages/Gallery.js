...
function Gallery() {
    ...
    // "/gallery?pageNum=x" 에서 pageNum을 추출하기 위한 hook
    // pageNum이 전달되지 않으면 기본값 pageNum=1 이 적용되고, 전달된다면 전달된 값이 적용된다.
    ...
    const refresh = (pageNum)=>{
        ...
        .then(res=>{
            ...
            // API 서버로 부터 받아온 데이터를 상태값으로 관리
            setPageInfo(res.data)
            // startPageNum, endPageNum 을 이용해서 pageArray 만들기(state 변경)
    ...
    /*
        ...
        useEffect(함수, [params]) -> component 활성화 시점, params 값이 변경될때 마다 호출된다.
        useEffect(함수, [state1, state2]) -> component 활성화 시점 또는 state1 or state2 값이 변경될때 마다 호출된다.
    */
    useEffect(()=>{
        // 갤러리 목록을 요청해서 읽어온다.
    ...
    //const [num, setNum]=useSearchParams({})
    ...
    return (
        <>
            <Link to="/gallery/new">이미지 업로드</Link>
            <h1>갤러리 목록</h1>
            <Row>
                {
                        ...
                        <Col sm={6} md={3} key={item.num}>
                            <Card>
                                ...
                                <Card.Body>
                                    ...
                                    <Button as={Link} to={`/gallery/${item.num}`}></Button>
            ...
            <Pagination className="mt-3">
                <Pagination.Item onClick={()=>{
                    ...
                    setParams({pageNum:pageInfo.startPageNum})
                }} disabled={pageInfo.pageNum === 1}>&laquo;</Pagination.Item>
                {
                    pageArray.map(num=> (
                        <Pagination.Item onClick={()=>{
                            // setParams() 함수를 이용해서 search parameter 값을 반영한다.
                            setParams({pageNum:num})
                        }} key={num} active={num === pageInfo.pageNum}>{num}</Pagination.Item>
                    ))
                }
                <Pagination.Item onClick={()=>{
                    setParams({pageNum:pageInfo.endPageNum})
                }} disabled={pageInfo.pageNum === pageInfo.endPageNum}>&raquo;</Pagination.Item>
            </Pagination>
        </>
    );
}
